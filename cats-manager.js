(function () {
  ...
  function renderMediaPreview() {
    ...
    state.mediaItems.forEach(function (item, index) {
      ...
      card.className =
        "preview-thumb relative aspect-square rounded-lg overflow-hidden bg-surface-container group";
      ...
      if (item.mediaType === "video") {
        ...
        iconWrap.innerHTML =
          '<span class="material-symbols-outlined text-tertiary text-4xl">videocam</span>';
        ...
        img.addEventListener("error", function () {
          ...
          readFileAsDataUrl(item.file)
            ...
            .catch(function () {
              // Leave empty preview if browser cannot decode this image type.
            });
        });
      ...
      controls.className =
        "absolute inset-x-1 top-1 flex items-center justify-between";

      var thumbBtn = document.createElement("button");
      thumbBtn.type = "button";
      thumbBtn.className =
        "bg-surface-container-lowest/90 text-on-surface rounded-full px-2 py-0.5 text-[10px] font-bold";
      thumbBtn.textContent =
        item.id === state.selectedMediaId ? "Thumbnail" : "Set thumb";
      ...
      removeBtn.className =
        "bg-error-container/90 text-on-error-container rounded-full w-6 h-6 flex items-center justify-center";
      removeBtn.innerHTML =
    ...
    return firestore
      ...
      .then(function (doc) {
        ...
        if (!doc.exists || !doc.data()) {
          container.innerHTML =
            "<p class='text-sm text-on-surface-variant'>No categories available</p>";
          return;
        }
        ...
        return calculateCategoryUsage().then(function (counts) {
          ...
          ids.forEach(function (id) {
            ...
            btn.className =
              "category-btn px-3 py-1.5 rounded-full text-sm font-medium border border-outline-variant bg-surface-container text-on-surface hover:bg-surface-container-high transition-all";
            btn.innerHTML =
              data[id] +
  ...
  function uploadFileItems(fileItems) {
    ...
    return new Promise(function (resolve, reject) {
      function next() {
        ...
        setStatus(
          "Uploading media " + (index + 1) + " of " + fileItems.length + "...",
          "info",
        );
    ...
    if (selectedItem) {
      thumbnailUrl =
        selectedItem.kind === "url"
          ? selectedItem.url
          : uploadedMap[selectedItem.id] || null;
    }
  ...
  function createOrUpdateCat() {
    ...
    uploadFileItems(fileItems)
      ...
      .then(function () {
        var successMsg =
          state.mode === "edit" ? "Update complete" : "Upload complete";
  ...
  function startEdit(catId) {
    firestore
      ...
      .then(function (doc) {
        ...
        var catIds = (data.categories || "").split(",").filter(function (x) {
          return !!x;
        });
        ...
        resetMediaState();
        var existingUrls =
          Array.isArray(data.medias) && data.medias.length
            ? data.medias.slice()
            : [];
  ...
  function loadCats() {
    ...
    return firestore
      ...
      .then(function (snapshot) {
        ...
        if (snapshot.empty) {
          html =
            "<p class='text-center text-on-surface-variant py-12'>No cats yet</p>";
        } else {
          snapshot.forEach(function (doc) {
            var data = doc.data();
            ...
            var catNames = catIds.map(function (id) {
              return state.categoryMap[id] || "Unknown";
            });

            html +=
              "<div class='flex gap-3 p-3 rounded-lg hover:bg-surface-container transition-colors group'>";
            if (thumb) {
              html +=
                "<img src='" +
                escapeHtml(thumb) +
            ...
            if (catNames.length > 0) {
              ...
              catNames.forEach(function (name) {
                html +=
                  "<span class='text-xs bg-secondary-fixed-dim text-secondary-dim px-2 py-0.5 rounded-full'>" +
                  escapeHtml(name) +
                  "</span>";
              });
              html += "</div>";
            ...
            html += "</div>";
            html +=
              "<button type='button' onclick='window.CatsManager.startEdit(\"" +
              doc.id +
              "\")' class='flex-shrink-0 p-2 text-tertiary hover:text-primary opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all'><span class='material-symbols-outlined'>edit</span></button>";
            html +=
              "<button type='button' onclick='window.CatsManager.deleteCat(\"" +
              doc.id +
              "\")' class='flex-shrink-0 p-2 text-tertiary hover:text-error opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all'><span class='material-symbols-outlined'>delete</span></button>";
            html += "</div>";
          });
        }
